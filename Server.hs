...
-----------------------------------------------------------------------------------------------------------------
-- tryChangeClient :: Server -> Client -> ClientName -> IO()
-- tryChangeClient server@Server{..} client_name name = atomically $ do
   --         clients <- readTVar serverClients
            -- clientName
      ...
      --      modifyTVar serverClients $ M.update (re client_name) name clients
        --    notify client_name $ "You got changed " 
            -- return (Just client)

--re :: Client ->( Client -> Maybe Client )
--re c@Client{..} =  return(Just c)  

--------------------------------------------------------------------------------------------------------------------
-- tryChangeClient :: Server -> Client 
...
handleMessage server client@Client{..} (Command msg) = case words msg of
   ...
   -- "/nick":newname    -> tryChangeClient server client (unwords newname)      >> return True
...
kick :: Server -> Client -> String -> String -> IO ()
kick Server{..} kicker@Client{..} kickee reason = atomically $ do
  ...
  where
    ...
    yesKick Client{..} = do
      -- getname kicker 
      writeTVar clientKicked $ Just (getname kicker, reason)
      notify kicker $ "You kicked " ++ clientName
